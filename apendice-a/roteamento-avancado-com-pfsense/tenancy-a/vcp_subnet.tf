...
# subnpub-1
resource "oci_core_subnet" "vcp_vcn-internet_subnpub-1" {
    provider = oci.vcp

    compartment_id = var.compartment_id
    vcn_id = oci_core_vcn.vcp_vcn-internet.id
    dhcp_options_id = oci_core_dhcp_options.vcp_vcn-internet_dhcp-options.id
    route_table_id = oci_core_route_table.vcp_vcn-internet_subnpub-1_route-table.id
    security_list_ids = [oci_core_security_list.vcp_vcn-internet_subnpub-1_secl.id]

    display_name = "subnpub-1"
    dns_label = "subnprv1"
    cidr_block = "10.100.60.128/25"
    prohibit_public_ip_on_vnic = false
}