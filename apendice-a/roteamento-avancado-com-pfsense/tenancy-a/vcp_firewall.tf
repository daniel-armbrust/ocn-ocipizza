...
resource "oci_core_vnic_attachment" "vcp_vm-firewall_vnic_wan-inbound" {    
    provider = oci.vcp
    ...
    create_vnic_details {    
        display_name = "vnic_wan-inbound"    
        hostname_label = "vcp-fw-win"
        private_ip = "10.100.20.94"        
        subnet_id = oci_core_subnet.vcp_vcn-firewall_subnpub-wan-inbound.id
        skip_source_dest_check = true
